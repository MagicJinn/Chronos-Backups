...
public final class ChronosTomlSpec {
    ...
    public static String renderDocument(ModConfig config) {
        return String.join("\n",
                ...
                "# Recommended value: 5. Values lower than 3 can be used to save space, but risks serious data loss if a catastrophic error occurs.",
                "# Values below 1 disable automatic removal.",
                KEY_MAX_STORED_BACKUPS + " = " + config.maxStoredBackups,
                "",
                "# Snapshot storage: \"zip\" or \"none\".",
                "# \"zip\" stores one archive; \"none\" stores an uncompressed folder (same basename).",
