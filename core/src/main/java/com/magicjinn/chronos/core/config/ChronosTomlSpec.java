...
public final class ChronosTomlSpec {
    ...
    private static final List<Entry> ENTRIES = Collections.unmodifiableList(Arrays.asList(
            string(KEY_BACKUP_FOLDER_NAME,
                    ...
                    "# Single folder name under the Minecraft run directory for backups.",
                    "# Must not contain path separators or \"..\". Invalid values fall back to \"backups\".",
                    "# Example: \"backups\""),
            ...
            integer(KEY_MAX_STORED_BACKUPS,
                    ...
                    "# After a successful backup, oldest local snapshots are removed if this would be exceeded.",
                    "# Cloud sync also trims remote Chronos backups to this cap.",
                    "# Recommended value: 5. Values lower than 3 save space but risk losing older restore points.",
            ...
            bool(KEY_SHOULD_KEEP_LOCAL_BACKUPS,
                    ...
                    "# When true, keep local backups after a successful cloud upload.",
                    "# When false, each backup is deleted locally once it has been uploaded."),
